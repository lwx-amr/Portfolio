import WSModel from '../repository/subsModel'
...
const listWorkSpaces = (req, res) => {
  WSModel.find({
    $or: [{ ownerID: req.params.id }, { 'users.id': req.params.id }, { emails: req.query.email }]
  })
...
const addNewUser = (req, res) => {
  WSModel.update({ _id: req.params.id }, { $push: { users: req.body.id } }, (err, data) => {
    if (err) res.send(err)
    res.json(data)
  })
}
...
const decNumOfJobs = (req, res) => {
  WSModel.findById(req.params.id, (err, data) => {
    ...
    if (data.numOfJobs > 0) {
      ...
      res.json({
        Empty_WS: 'This workspace either has no job profiles yet or they are already deleted'
      })
  })
}
